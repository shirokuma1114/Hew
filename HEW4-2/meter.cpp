...
// マクロ定義
//*****************************************************************************名
#define	Meter_SIZE_X		Texture_GetWidth(TEXTURE_INDEX_Meter)							// スコアの数字の幅
#define	Meter_SIZE_Y		Texture_GetHeight(TEXTURE_INDEX_Meter)							// スコアの数字の高さ
#define	Meter_INTERVAL_X	(0.0f)															// スコアの数字の表示間隔

#define	NUM_PLACE			(8)																// スコアの桁数

#define	Meter_POS_X			(SCREEN_WIDTH - (Meter_INTERVAL_X + Meter_SIZE_X) * NUM_PLACE - 10.0f)	// スコアの表示基準位置Ｘ座標
#define	Meter_POS_Y			(10.0f)																	// スコアの表示基準位置Ｙ座標
...
// グローバル変数宣言
//*****************************************************************************
LPDIRECT3DTEXTURE9		g_pTextureMeter = NULL;	// テクスチャへのポインタ
LPDIRECT3DVERTEXBUFFER9 g_pVtxBuffMeter = NULL;	// 頂点バッファへのポインタ

D3DXVECTOR3				g_posMeter;				// 位置
D3DXVECTOR3				g_rotMeter;				// 向き
...
HRESULT MakeVertexMeter(LPDIRECT3DDEVICE9 pDevice)
{
	...
	if (FAILED(pDevice->CreateVertexBuffer(sizeof(VERTEX_2D) * NUM_VERTEX * NUM_PLACE,	// 頂点データ用に確保するバッファサイズ(バイト単位)
		...
		&g_pVtxBuffMeter,						// 頂点バッファインターフェースへのポインタ
		NULL)))									// NULLに設定
	{
		return E_FAIL;
	}
